...
- docs: |deployment recovery| Add a frozen-baseline / do-not-roll recovery pattern to `skills/johnsonmail-deploy/SKILL.md`, `DEPLOYMENT_CHECKLIST.md`, and `DEPLOYMENT_FOR_AGENTS.md`: every real environment should maintain a known-good baseline, historical restores must recover the known-good deploy/artifact first, and `/`, `/?jwt=...`, and `/open_api/settings` must be verified separately before applying any small delta. Also add `ops/KNOWN_GOOD_BASELINE.example.md` as the baseline template, including the safe sequence for “keep current jwt UI, only patch settings” operations
- docs: |interface spec| Add `ops/INTERFACE_SPEC.md` as the single repo-wide source of truth for the default mailbox UI. The default spec is now the current tabbed mailbox UI (Address Management / Mailbox / Account Settings / Appearance / current-page filter / auto-refresh). Historical or compatibility UIs are no longer treated as parallel default targets and are only kept as compatibility modes or environment-baseline notes
